...
!function(){"use strict";var content=document.querySelector(".post-content"),bar=document.querySelector(".time-bar");if(!content||!bar)return;var completed=bar.querySelector(".completed"),remaining=bar.querySelector(".remaining"),completedLabel=bar.querySelector(".time-completed"),remainingLabel=bar.querySelector(".time-remaining"),minutes=Math.max(1,parseInt(bar.getAttribute("data-minutes"),10)||1),lastScroll=window.pageYOffset||document.documentElement.scrollTop;function formatTime(totalSeconds){totalSeconds=Math.max(0,Math.round(totalSeconds));var mins=Math.floor(totalSeconds/60),secs=totalSeconds%60;return(mins<10?"0":"")+mins+":"+(secs<10?"0":"")+secs}function updateProgress(){var scrollTop=window.pageYOffset||document.documentElement.scrollTop,maxScroll=Math.max(1,document.documentElement.scrollHeight-window.innerHeight),progress=Math.min(1,Math.max(0,scrollTop/maxScroll));completed.style.width=(progress*100).toFixed(2)+"%";remaining.style.width=((1-progress)*100).toFixed(2)+"%";var totalSeconds=minutes*60,elapsedSeconds=totalSeconds*progress;completedLabel.innerText=formatTime(elapsedSeconds);remainingLabel.innerText=formatTime(totalSeconds-elapsedSeconds);bar.style.bottom=scrollTop>80&&progress<.995?"0%":"-100%";var eventName=progress>=1?"finishedReading":scrollTop>80?"stillReading":"readingStarted",event=document.createEvent("CustomEvent");event.initCustomEvent(eventName);document.dispatchEvent(event);lastScroll=scrollTop}document.addEventListener("scroll",updateProgress,{passive:true});window.addEventListener("resize",updateProgress);updateProgress()}();
...
;(function(c,l,a,r,i,t,y){c[a]=c[a]||function(){(c[a].q=c[a].q||[]).push(arguments)};t=l.createElement(r);t.async=1;t.src="https://www.clarity.ms/tag/"+i;y=l.getElementsByTagName(r)[0];y.parentNode.insertBefore(t,y)})(window,document,"clarity","script","yhnqohr3py");

;(function(){var host=location.hostname;if(host!=="zihanhan.work"&&host!=="www.zihanhan.work"){return}var send=function(){fetch("https://analytics.zihanhan.work/hit",{method:"POST",mode:"cors",keepalive:true,headers:{"Content-Type":"application/json"},body:JSON.stringify({path:location.pathname+location.search,referrer:document.referrer||""})}).catch(function(){})};if(document.readyState==="complete"){send()}else{window.addEventListener("load",send,{once:true})}})();
